...
// - No MMU, no interrupts/CSRs, no compressed (C) extension
//
// 日本語概要:
...
static void dump_regs(const cpu_t *cpu) {
    for (int i = 0; i < 32; i++) {
        fprintf(stderr, "x%02d=%016" PRIx64 "%s", i, cpu->x[i], ((i%4)==3)?"\n":" ");
    }
}
// 日本語: レジスタのトレース出力。
...
static void step(cpu_t *cpu, mem_t *mem, const run_opts_t *opt) {
    uint64_t pc = cpu->pc;
    uint32_t inst = mem_read32(mem, pc);
    if ((inst & 3) != 3) {
    ...
    switch (d.opcode) {
        ...
        case 0x03: { // LOAD
            ...
            switch (d.funct3) {
                case 0: rd_val = (uint64_t)(int64_t)(int8_t)mem_read8(mem, addr); break; // LB
                case 1: rd_val = (uint64_t)(int64_t)(int16_t)mem_read16(mem, addr); break; // LH
                case 2: rd_val = (uint64_t)(int64_t)(int32_t)mem_read32(mem, addr); break; // LW
                case 3: rd_val = mem_read64(mem, addr); break; // LD
                case 4: rd_val = (uint64_t)mem_read8(mem, addr); break; // LBU
                case 5: rd_val = (uint64_t)mem_read16(mem, addr); break; // LHU
                case 6: rd_val = (uint64_t)mem_read32(mem, addr); break; // LWU
        ...
        case 0x23: { // STORE
            ...
            switch (d.funct3) {
                case 0: mem_write8(mem, addr, (uint8_t)(rs2 & 0xff)); break; // SB
                case 1: mem_write16(mem, addr, (uint16_t)(rs2 & 0xffff)); break; // SH
                case 2: mem_write32(mem, addr, (uint32_t)(rs2 & 0xffffffffu)); break; // SW
                case 3: mem_write64(mem, addr, rs2); break; // SD
        ...
        case 0x73: { // SYSTEM
            ...
            if (csr_funct == 0 && (d.raw >> 20) == 0) { // ECALL
                cpu->halt = true;
                cpu->exit_code = cpu->x[10]; // a0
            } else if (csr_funct == 0 && (d.raw >> 20) == 1) { // EBREAK
                cpu->halt = true;
                cpu->exit_code = 0;
            } else {
